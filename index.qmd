...
url = f"https://api.open-meteo.com/v1/meteofrance?latitude=43.6109&longitude=3.8763&hourly=temperature_2m,weathercode,windspeed_10m&daily=weathercode,temperature_2m_max,temperature_2m_min,precipitation_sum&timezone=Europe%2FLondon&start_date={debut}&end_date={fin}"
...
for i in range(7):
    if sumpluie[i] == None:
       sumpluie[i] = 'Pas encore de données ' 
...
for i in range(7):
    for j in range(24):
        if vent[k] == None:
            tabvent[i,j] = tabvent[i,j-1] #fausse le moins les résultats selon mes tests
        else :
            tabvent[i,j] = vent[k]
        k+=1
